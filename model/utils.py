...
def postprocess_args(args):
    ...
    svname = '{}_{}_{}_way_{}_shot_{}_{}'.format(args.backbone_class, args.dataset, args.way, args.shot, current_time,
                                                 args.cap)

    save_path = os.path.join('/home/ubuntu/code/Work3/save_TMCN', svname)
    args.save_path = save_path
    print(args.save_path)
    ensure_path(args.save_path)
    args.log_path = os.path.join(save_path, 'log.txt')
    log = open(args.log_path, 'a+')
    # parser = get_command_line_parser()
    # args = parser.parse_args()
    logargs = vars(args)
    pp = pprint.PrettyPrinter()
    logargs = pp.pformat(logargs)
    log.write(logargs)
    log.write('\n\n')
    log.close()
...
def get_command_line_parser():
    ...
    parser.add_argument('--backbone_class', type=str, default='Res12',
                        choices=['ConvNet', 'Res10', 'Res12', 'Res18', 'WRN'])
    ...
    parser.add_argument('--manual_seed', type=int, default=1156)
    ...
    parser.add_argument('--num_grid', type=int, default=9)
    parser.add_argument('--num_edge', type=int, default=3)
    return parser
